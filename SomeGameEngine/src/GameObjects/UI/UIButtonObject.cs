...
public class UIButtonObject : UIRectObject
{
    ...
    public UIButtonObject(Color baseColor, Color hoveredColor, Color clickColor)
    {
        BaseColor = baseColor;
    ...
    protected override void OnButton(byte button, bool up, int mouseX, int mouseY)
    {
        if (button == 1)
        {
            ...
            if (up && _clicking)
            {
                _clicking = false;
                if (_hovered)
                    Clicks(mouseX - Position.X, mouseY - Position.Y);
            }
        }
    }
}